...
def main() -> None:
    ...
    st.caption(
        "Prueba local de inferencia usando CSV sintético y los metodos de src/inference."
    )
    ...
    col_b.metric(
        "Success",
        int((station_status["status"] == "success").sum())
        if not station_status.empty
        else 0,
    )
    col_c.metric(
        "Skipped",
        int((station_status["status"] == "skipped").sum())
        if not station_status.empty
        else 0,
    )
    ...
    station_options = sorted(
        {int(result["station_id"]) for result in inference_results}
    )
    selected_station = st.selectbox(
        "Estacion", station_options, format_func=lambda value: f"Station {value}"
    )
    selected_horizon = st.selectbox(
        "Horizonte", [6, 12], index=0, format_func=lambda value: f"{value}h"
    )

    selected_result = next(
        result
        for result in inference_results
        if int(result["station_id"]) == selected_station
        and int(result["horizon_hours"]) == selected_horizon
    )
    input_points = payload["station_inputs"].get(selected_station, [])
    fig = build_plot(
        input_points=input_points,
        prediction_points=selected_result["predictions_json"]["points"],
    )
