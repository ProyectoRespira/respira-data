...
create table if not exists ops.inference_runs (
    id uuid primary key,
    flow_run_id text not null,
    deployment text null,
...
create index if not exists idx_inference_runs_as_of on ops.inference_runs (as_of);
create index if not exists idx_inference_runs_status on ops.inference_runs (status);

create table if not exists ops.inference_station_status (
    id uuid primary key,
    inference_run_id uuid not null references ops.inference_runs(id),
...
create table if not exists ops.inference_results (
    id uuid primary key,
    inference_run_id uuid not null references ops.inference_runs(id),
    station_id bigint not null,
    as_of timestamptz not null,
...
create index if not exists idx_inference_results_run_id on ops.inference_results (inference_run_id);
create index if not exists idx_inference_results_station on ops.inference_results (station_id);
create index if not exists idx_inference_results_as_of on ops.inference_results (as_of);
