...
- Reactivating in the backoffice writes `value='active'`, which drops the row out of this model. The station's normal derived status applies again on the next run, so it comes back **only if** its source still reports it active and it has reported recently — an override can hold a station off, but cannot bring a silent sensor back.
- `processed` on the table is not used for filtering: an override is a desired state, not an event, so it has to keep applying on every run.
- **DDL ownership.** This pipeline provisions the table; respira-webapp operates it. `warehouse_bootstrap` runs `pipelines/sql/04_station_overrides_table.sql` as `dbtuser`, creating `respira_gold.station_overrides` and granting the backend role (`BACKEND_DB_ROLE`, default `backenduser`) SELECT/INSERT/UPDATE/DELETE plus schema USAGE — never CREATE. That split is deliberate: the pipeline establishes its own runtime dependency before the backend is deployed, and the backend never needs schema-wide table-creation privileges. Failure to provision fails the bootstrap rather than warning, so the flow cannot finish green with the relation missing.
- The SQL definition must stay compatible with Django's `StationOverride` model — columns, types, nullability, constraint and index names. It was derived from `manage.py sqlmigrate api 0005_stationoverride`, not written by hand, and `tests/test_station_overrides_provisioning.py` pins the contract. Note `id` is an IDENTITY column (Django 4.2 emits `GENERATED BY DEFAULT AS IDENTITY`), so there is no separate sequence to grant on.
